...
Frame *FrameFactory::createFrame(const ByteVector &origData, Header *tagHeader) const
{
  ...
  if(frameID == "COMM") {
	debug("Comments frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
  ...
  if(frameID == "RVA2") {
	debug("RVA2 frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
  ...
  if(frameID == "UFID") {
	debug("UFID frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
  ...
  if(frameID == "GEOB") {
    debug("GEOB frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
  ...
  if(frameID.startsWith("W")) {
	debug("URL Link frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
  ...
  if(frameID == "USLT") {
	debug("Lyric/text frames are currently disabled.");
    return new UnknownFrame(data, header);
  }
