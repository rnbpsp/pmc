...
{
  String null;
  d->year = i > 0 ? String::number(i) : null;
}
...
void ID3v1::Tag::parse(const ByteVector &data)
{
  ...
  d->title = String(data.mid(offset, 30), String::Latin1).stripWhiteSpace();
  offset += 30;
  d->artist = String(data.mid(offset, 30), String::Latin1).stripWhiteSpace();
  offset += 30;
  d->album = String(data.mid(offset, 30), String::Latin1).stripWhiteSpace();
  offset += 30;
  d->year = String(data.mid(offset, 4), String::Latin1).stripWhiteSpace();
  offset += 4;
  ...
  if(data[offset + 28] == 0 && data[offset + 29] != 0) {
    // ID3v1.1 detected
    d->comment = String(data.mid(offset, 28), String::Latin1).stripWhiteSpace();
